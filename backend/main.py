...
DRIVE_FILES = {
    "hit_predictor.pkl": "154mzR1ZjbXzDEkP2ewMaUV2uvUU6Axke",
    "rec_data.csv":      "10Trtvm_jOLXC6jxkCkioElrZYIybU6NA",
}
...
def predict_hit(features: SongFeatures):
    input_data = [[
        ...
        features.duration_ms, features.genre_encoded
    ]]
...
def search_songs(query: str):
    ...
    return {"results": results[['track_name', 'artists',
                                'track_genre', 'popularity']].to_dict(orient='records')}

@app.get("/genres")
def get_genres():
    return {"genres": sorted(rec_df['track_genre'].unique().tolist())}